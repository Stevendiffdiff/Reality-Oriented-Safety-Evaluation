...
# %%
set_seed(args.seed)
LOG_TITLE = f"baseline_CALM_{args.seed}_{args.data_path[-8:-4]}_{args.system_prompt}_{args.victim_model}"
consis_thre = 0.
div_thre = 0.4
DELAY_STEP = 0
...
def collate(batch: dict, step: int = 0):
    ...
    batch['non_gibberish'] = nonGibberish_judge.reward_fn([_p[:min(80, len(_p))] for _p in prompts])
