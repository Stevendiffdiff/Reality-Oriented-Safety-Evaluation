...
from utils.common import weight_init, str2bool
...
parser.add_argument("--delay_step", type=int, default=0)
...
class TopicDiverseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        prompt = (
            f"Write a request in less than {generation_kwargs['max_new_tokens']} tokens TO prompt a LLM to generate "
...
def compute_and_save_final_results():
    def cos_dist(X, Y):
    ...
    topic_tensor = torch.tensor(topic_embeddings).to(device_config['train'])
    ...
    for key, values in records.items():
        try:
            plt.plot(values)
            plt.title(f"{key} - {log_prefix}")
            plt.savefig(f"{log_dir}/{key}.png")
            plt.clf()
        except:
            plt.clf()
            continue
    writer.close()
    pd.DataFrame(records).to_csv(training_log_file, index=True)
