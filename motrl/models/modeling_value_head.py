...
class ValueHead(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        # self.summary = nn.Sequential(nn.Linear(hidden_size, 1))
        if 'v_dim' in kwargs.keys():
            self.summary = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Linear(hidden_size, kwargs['v_dim']+4))
        else:
            self.summary = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Linear(hidden_size, 1))
    ...
    def forward(self, hidden_states):
        ...
        if output.dtype != self.summary[-1].weight.dtype:
            output = output.to(self.summary[-1].weight.dtype)
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        # if "is_peft_model" in kwargs.keys():
        #     self.is_peft_model = kwargs['is_peft_model']
    ...
    def forward(
        ...
        **kwargs,
        ...
        # TODO: for detached value loss. If you want to set back, just delete the ```with self.pretrained_model.disable_adapter(): ... hidden_state[-1]``` part.
        with torch.no_grad():
            with self.pretrained_model.disable_adapter():
            ...
            if last_hidden_state.device != self.v_head.summary[-1].weight.device:
                last_hidden_state = last_hidden_state.to(self.v_head.summary[-1].weight.device)
