...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        morl_config: Optional[dict] = None,
    ):
        """
...
## TODO HERE
        def trivial_gen(*args, **kwargs):
            i = 0
            while True:
                i += 1
                yield min(1., tanh(i))

        def trivial_distribution(*args, **kwargs):
            for _ in range(config.steps):
                yield torch.tensor([1. / self.model.v_head.summary[-1].out_features for _ in range(self.model.v_head.summary[-1].out_features)], device=self.current_device)

## TODO HERE        
        self.lambda_generator = trivial_gen() if self.config.v_lambda is None else self.config.v_lambda
        self.w_generator = trivial_distribution() if self.config.reward_coef is None else self.config.reward_coef
        self._lambda = 0.
...
## TODO HERE: conpute_rewards() function
                rewards, non_score_reward, kls = self.compute_rewards(
                    scores, active_full_logprobs, ref_full_logprobs, masks
                )
            else:
                rewards, non_score_reward, kls = self.compute_rewards(
                    scores, all_logprobs, ref_logprobs, masks, batch=batch
                )
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            logprobs = logprobs_from_logits(logits[:, :-1, :], input_ids[:, 1:])# gather == True, so logprobs only contains the probs of tokens in the sentence
    ...
    def train_minibatch(
        ...
        # if pg_coef > 0.01:
        #     loss = pg_coef * loss_p + vf_coef * loss_v - entropy_coef * loss_e
        # else:
        #     # self.model.eval()
        loss = pg_coef * loss_p + vf_coef * loss_v - entropy_coef * loss_e
        train_stats['loss/total'] = loss.detach()
        self.accelerator.backward(loss)
        # for name, para in self.model.v_head.named_parameters():
        #     print(f"Post Gradient: {name}, {para.grad.norm().item()}")

        # for name, param in self.model.named_parameters():
        #     if param.requires_grad and param.grad is not None and name.__contains__("lora"):
        #         grad_norm = param.grad.norm().item()
        #         if grad_norm > 1e-6:
        #             print(f"here, the policy is trained! {name}: grad norm = {grad_norm:.6f}")
        ...
        if self.config.max_grad_norm is not None:
            if self.accelerator.sync_gradients:
                self.accelerator.clip_grad_norm_(
                    [p for p in self.model.parameters() if p.grad is not None], self.config.max_grad_norm
                )
        self.optimizer.step()
    ...
    def compute_rewards(
        ...
        for i, (score, logprob, ref_logprob, mask) in enumerate(zip(scores, logprobs, ref_logprobs, masks)):
            reward = torch.zeros([logprob.shape[0], score.shape[0]+4], dtype=float, device=self.current_device)
            # compute KL penalty (from difference in logprobs)
            kl = self._kl_penalty(logprob, ref_logprob)
            kls.append(kl)
            non_score_reward = -kl
            non_score_rewards.append(non_score_reward)
            reward[:, 0] += non_score_reward
            last_non_masked_index = mask.nonzero()[-1]

            # reward is preference model score + KL penalty
            # TODO: compute token-level rewards
            ## TODO HERE: change rewards into vectors            
            reward[last_non_masked_index, 4:] += score
            reward[mask.nonzero().squeeze(), 1] += batch["state_entropy"][i]
            reward[mask.nonzero().squeeze(), 2] += batch["policy_cover"][i]
            reward[mask.nonzero().squeeze(), 3] += batch["t_rnd"][i]
    ...
    ## TODO HERE: def a new function to mask (because values and advantages are dimentionally different to masks)
        def broad_cast_mask(scores, mask):
            _shape = mask.shape
            if len(_shape)<len(scores.shape):
        ...
        lastgaelam = 0
        advantages_reversed = []
        gen_len = rewards.shape[-2]

        values = broad_cast_mask(values, mask)
        rewards = broad_cast_mask(rewards, mask)

        if self.config.whiten_rewards:
            rewards = masked_whiten(rewards, mask, shift_mean=False)

        for t in reversed(range(gen_len)):
            nextvalues = values[:, t + 1, :] if t < gen_len - 1 else torch.zeros_like(values[0, 0, :], dtype=float, device=self.current_device)
...
## TODO HERE: a funcion to weight advantages and values
    def weight_tensor(self, weight: torch.FloatTensor, values: torch.FloatTensor):
        _shape = values.shape[:-1]
        _weight = weight.clone()
        for k in reversed(range(len(_shape))):
            _temp = _weight
            _weight = torch.stack([_temp for _ in range(_shape[k])])
        return _weight.view(values.shape) * values
    ...
    def loss(
        ...
        w = next(self.w_generator)
        # _lambda = next(self.lambda_generator)
        ## TODO HERE: LA and LB
        # LA
        vf_losses1 = (vpreds - returns).square()
        vf_losses2 = (vpredclipped - returns).square()
        vf_lossA = 0.01 * masked_mean(torch.max(vf_losses1, vf_losses2).sum(axis=-1), mask)
        vf_clipfrac = masked_mean(torch.gt(vf_losses2, vf_losses1).float(), mask)

        # LB
        vf_lossB = 0.5 * masked_mean(self.weight_tensor(w, (vpredclipped - returns)) ** 2, mask)

        # Policy Loss
        ratio = torch.exp(logprobs - old_logprobs)

        _advantages = self.weight_tensor(w, advantages).sum(dim=-1)
        pg_losses = -_advantages * ratio
        pg_losses2 = -_advantages * torch.clamp(ratio, 1.0 - self.config.cliprange, 1.0 + self.config.cliprange)

        pg_loss = masked_mean(torch.max(pg_losses, pg_losses2), mask)
        pg_clipfrac = masked_mean(torch.gt(pg_losses2, pg_losses).float(), mask)

        vf_loss = (1-self._lambda) * vf_lossA + self._lambda * vf_lossB

        # Examine the loss. If too large, skip the batch.
        ...
        with torch.no_grad():
            approxkl_new = masked_mean((ratio - 1) - ratio.log(), mask)
        ...
        stats = dict(
            loss=dict(policy=pg_loss.detach(), value=vf_loss.detach(), value_A=vf_lossA.detach(), value_B=vf_lossB.detach()),
            policy=dict(
                entropy=entropy.detach(),
                approxkl=approxkl.detach(),
                approxkl_new=approxkl_new.detach(),
                policykl=policykl.detach(),
                clipfrac=pg_clipfrac.detach(),
                advantages=_advantages.detach(),
                advantages_mean=masked_mean(_advantages, mask).detach(),
                ratio=ratio.detach(),
            ),
        ...
        return pg_loss, vf_loss, entropy, flatten_dict(stats)
