...
def masked_mean(values: torch.Tensor, mask: torch.Tensor, axis: Optional[bool] = None, all: bool = True) -> torch.Tensor:
    ...
    def broad_cast_mask(scores, mask):
            ...
            return scores * mask
    if axis is not None:
        return (broad_cast_mask(values, mask)).sum(axis=axis) / mask.sum(axis=axis)
    elif all:
        return (broad_cast_mask(values, mask)).sum() / mask.sum()
    else:
        return (broad_cast_mask(values, mask)).sum(axis=0).sum(axis=0) / mask.sum()


def masked_var(values: torch.Tensor, mask: torch.Tensor, unbiased: bool = True) -> torch.Tensor:
    """Compute variance of tensor with masked values."""
    mean = masked_mean(values, mask, all=False)
    centered_values = values - mean
    variance = masked_mean(centered_values**2, mask, all=False)
    if unbiased:
        mask_sum = mask.sum()
        if mask_sum == 0:
            raise ValueError(
        ...
        bessel_correction = mask_sum * 1. / (mask_sum - 1)
...
def masked_whiten(values: torch.Tensor, mask: torch.Tensor, shift_mean: bool = True) -> torch.Tensor:
    ...
    mean, var = masked_mean(values, mask, all=False), masked_var(values, mask)
    whitened = (values - mean) * torch.rsqrt(var + 1e-8)
    if not shift_mean:
        whitened += mean
    return whitened
