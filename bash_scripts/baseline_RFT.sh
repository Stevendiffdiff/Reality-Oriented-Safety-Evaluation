        ...
        for system_prompt in "False" "True"; do
            for victim_model in "qwen-turbo" "gpt4o" "gemini" "deepseek"; do
                for method in "RFT" "CRT" "DiveRCT" "CALM"; do 
                    ...
                    echo '...baseline name='$method

                    accelerate launch --config_file <ACCELERATE-CONFIG-FILE> \
                    Reality-Oriented-Safety-Evaluation/scripts/baseline_RFT.py \
                    ...
                    --seed ${seed} --system_prompt ${system_prompt} --victim_model ${victim_model} --jailbreak_template ${jailbreak_template} --method ${method} \
                    --div_threshold ${div_threshold} \
                    > Reality-Oriented-Safety-Evaluation/logs/outputs/${method}_${seed}_${data_path##*/}_${system_prompt}_${victim_model}.txt
                done
            done
        done    
    done
