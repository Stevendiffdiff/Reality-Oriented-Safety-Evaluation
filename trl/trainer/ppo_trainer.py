...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.entropy_coef_generator = None if isinstance(config.entropy_coef, float) else config.entropy_coef
    ...
    def step(
        ...
        with torch.no_grad():
            ...
            if full_kl_penalty:
                ...
                rewards, non_score_reward, kls = self.compute_rewards(
                    scores, all_logprobs, ref_logprobs, masks, batch=batch, im_config=im_config
                )
    ...
    def compute_rewards(
        ...
        for i, (score, logprob, ref_logprob, mask) in enumerate(zip(scores, logprobs, ref_logprobs, masks)):
            ...
            reward[mask.nonzero().squeeze()] += im_config.se_coef * batch["state_entropy"][i]
            reward[mask.nonzero().squeeze()] += im_config.pc_coef * batch["policy_cover"][i]
            reward[mask.nonzero().squeeze()] += im_config.t_rnd_coef * batch["t_rnd"][i]
    ...
    def loss(
        ...
        entropy_coef = self.config.entropy_coef if not self.entropy_coef_generator else next(self.entropy_coef_generator)
        ...
        return pg_loss, self.config.vf_coef * vf_loss, -entropy_coef * entropy, flatten_dict(stats)
